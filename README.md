# cryrcit/debian-openldap

[![Docker Pulls](https://img.shields.io/docker/pulls/cryrcit/debian-openldap.svg)](https://hub.docker.com/r/cryrcit/debian-openldap/)
[![Docker Stars](https://img.shields.io/docker/stars/cryrcit/debian-openldap.svg)](https://hub.docker.com/r/cryrcit/debian-openldap/)
[![Layers](https://images.microbadger.com/badges/image/cryrcit/debian-openldap.svg)](https://hub.docker.com/r/cryrcit/debian-openldap/)

This project is generated from the original project of osixia if you want take the original please see this link : [Osixia](https://github.com/osixia/debian-openldap)

Latest release: 10.13.x - [OpenLDAP 2.4.57](https://www.openldap.org/software/release/changes.html) -  [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/cryrcit/debian-openldap/) 
...
- [cryrcit/openldap](#cryrcitopenldap)
	...
	- [Advanced User Guide](#advanced-user-guide)
		- [Extend cryrcit/openldap:10.13.x image](#extend-cryrcitopenldap150-image)
		- [Make your own openldap image](#make-your-own-openldap-image)
		- [Tests](#tests)
		- [Kubernetes](#kubernetes)
		- [Under the hood: cryrcit/light-baseimage](#under-the-hood-cryrcitlight-baseimage)
	- [Security](#security)
...
- Help new users with [issues](https://github.com/cryrcit/debian-openldap/issues) they may encounter
...
```sh
docker run --name my-openldap-container --detach cryrcit/openldap:10.13.x
```
...
```sh
docker run -p 389:389 -p 636:636 --name my-openldap-container --detach cryrcit/openldap:10.13.x
```
...
docker run \
	...
	--detach cryrcit/openldap:10.13.x
```
...
docker run \
	...
	cryrcit/openldap:10.13.x --copy-service
...
> [cryrcit/openldap-backup](https://github.com/cryrcit/debian-openldap-backup)
...
```sh
docker run --hostname ldap.my-company.com --detach cryrcit/openldap:10.13.x
```
...
Add --env LDAP_TLS=false to the run command:
	...
	docker run --env LDAP_TLS=false --detach cryrcit/openldap:10.13.x

### Multi master replication
Quick example, with the default config.

	#Create the first ldap server, save the container id in LDAP_CID and get its IP:
	LDAP_CID=$(docker run --hostname ldap.example.org --env LDAP_REPLICATION=true --detach cryrcit/openldap:10.13.x)
	LDAP_IP=$(docker inspect -f "{{ .NetworkSettings.IPAddress }}" $LDAP_CID)

	#Create the second ldap server, save the container id in LDAP2_CID and get its IP:
	LDAP2_CID=$(docker run --hostname ldap2.example.org --env LDAP_REPLICATION=true --detach cryrcit/openldap:10.13.x)
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] cryrcit/openldap:10.13.x --copy-service
...
```sh
docker run --detach cryrcit/openldap:10.13.x --loglevel debug
```

See all command line options:

```sh
docker run cryrcit/openldap:10.13.x --help
```
...
- **LDAP_REPLICATION_HOSTS**: list of replication hosts, must contain the current container hostname set by --hostname on docker run command. Defaults to :
  ...
  - ldap://ldap2.example.org
	...
	If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env LDAP_REPLICATION_HOSTS="#PYTHON2BASH:['ldap://ldap.example.org','ldap://ldap2.example.org']" --detach cryrcit/openldap:10.13.x
...
### Extend cryrcit/openldap:10.13.x image
...
See complete example in **example/extend-cryrcit-openldap**
...
```sh
git clone https://github.com/cryrcit/debian-openldap
cd debian-openldap
```
...
cryrcit-openldap kubernetes examples are available in **example/kubernetes**

### Under the hood: cryrcit/light-baseimage

This image is based on cryrcit/light-baseimage.
...
https://github.com/cryrcit/docker-light-baseimage

## Security
If you discover a security vulnerability within this docker image, please send an email to the cryrcit! team at security@cryrcit.net. For minor vulnerabilities feel free to add an issue here on github.
